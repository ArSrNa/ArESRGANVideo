...
var AutoProcess = {
  toFrame: function () {
    attoFrameWs.send(
      JSON.stringify({
        ...
        colorMatrix: "scale=in_color_matrix=bt709:out_color_matrix=bt601",
      })
    );
  },
  ...
  opt: function () {
    atoptWs.send(
      JSON.stringify({
        command: true,
        model: $("#AutoModel").val().split(",")[1],
        scale: $("#AutoModel").val().split(",")[0],
        path: $("#filePathText").html(),
      })
    );
  },
...
attoVideoWs.onmessage = function (data) {
  if (autoProgress(data, 3, "帧合成为视频")) {
    alert("处理完成");
    $("#autoPause").addClass("disabled");
    $("#autoStart").removeClass("disabled");
  }
};
