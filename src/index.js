...
ffmpegPath = path.join(process.cwd(), '/resources/extraResources','ffmpeg.exe');
esrganPath = path.join(process.cwd(), '/resources/extraResources','realesrgan-ncnn-vulkan.exe');
...
eapp.get('/hotUpdate', function(req, res) {
	return false;
	downCos('app.asar', path.join(__dirname)
		.replace('app\\src', ''))
})
